...
for i in range(n_test):
    ...
    data_bitflip_y = LDPclient.release_bitflip(data_y, alphabet_size, privacy_level)
    del data_y
    server_elltwo.load_private_data_multinomial_y(data_bitflip_y, alphabet_size)
    del data_bitflip_y

    data_z = data_gen.generate_multinomial_data(p2, sample_size)
    data_bitflip_z = LDPclient.release_bitflip(data_z, alphabet_size, privacy_level)
    del data_z
    server_elltwo.load_private_data_multinomial_z(data_bitflip_z, alphabet_size)
    del data_bitflip_z
    ...
    print(f"pval: {p_value_array[i,0]} -- {i+1}th test, time elapsed {t_end_i} -- emperical power so far: {(p_value_array[0:i] < significance_level).mean()}")
