...
for i in range(n_test):
    ...
    data_genRR_y = LDPclient.release_genRR(data_y, alphabet_size, privacy_level)
    del data_y

    data_z = data_gen.generate_multinomial_data(p2, sample_size)
    data_genRR_z = LDPclient.release_genRR(data_z, alphabet_size, privacy_level)
    del data_z

    
    server_elltwo.load_private_data_multinomial(data_genRR_y, data_genRR_z, alphabet_size)
    del data_genRR_y
    del data_genRR_z
...
with open('./p_value_array_priv_genrr_ell2_priv_' + str(int(privacy_level*10)) +'_k_2000_n_' + str(int(sample_size)) + 'high_sig.npy', 'wb') as f:
